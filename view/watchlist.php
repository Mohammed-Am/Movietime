...
<body>
    ...
    <form action="" method="post">
        <!--========== CONTENTS ==========-->
        <main>
            <h3 class="text-secondary m-5"> Watchlist </h3>
            <?php
            $con = conect();

            $sql = "SELECT count(*) FROM playList WHERE idUser=" . $_SESSION["idUser"];
            $mv1 = $con->query($sql)->fetchAll();
            if ($mv1[0][0] == 0) { ?>
                <h3 class="text-secondary m-5 text-center"> Your Watchlist is faild </h3>
                <?php
            } else {
                $sql = "SELECT * FROM playList WHERE idUser=" . $_SESSION["idUser"];
                $mv = $con->query($sql)->fetchAll(PDO::FETCH_ASSOC);

                foreach ($mv as $row) {
                    // echo $row['idMovies'], $_SESSION['idUser'];
                ?>
                    <div class="card bg-dark" style="width:15rem;">
                        <img class="card-img-top img-fluid " src="view/image/<?= $row['image'] ?>" alt="card image">
                        <div class="card-body">
                            <p><i class='bx bxs-star   nav__icon' style="color: gold ;"></i><span style="font-weight: bold;">8.3</span></p>
                            <p class="card-title">title: <?= $row['title']; ?></p>
                            <p class="card-text">type: <?= $row['type']; ?></p>
                            <!-- <p class="card-text">date:</p> -->
                            <p class="card-text">time: <?= $row['time']; ?></p>
                            <a href="?action=del&id=<?= $row['idMovies'] ?>"><input type="submit" name="del" class="btn btn-primary" value="Delete"></a>
                        </div>
                    </div>
                    <!-- Bootstrap 5 Scripts -->
...
<?php  }
            ...
            if (isset($_POST['del'])) {
                $id = $row['idMovies'];
                $idU = $_SESSION['idUser'];
                $sql = ' DELETE FROM  playList where idMovies=:A AND idUser=:B';
                $stmt = $con->prepare($sql);
                $stmt->execute(array('A' => $id, 'B' => $idU));
            }
?>
</section>
